...
public enum JavaGlossary {
    ...
    static let words: [String: String] = [
        ...
        "HttpServer": "Ein kleiner Webserver aus dem JDK: nimmt Anfragen an und schickt Antworten zurück.",
        "HttpExchange": "Ein einzelner Austausch im Server: die angekommene Anfrage und die Antwort darauf.",
        "HttpClient": "Verschickt Anfragen an einen Server und wartet auf die Antworten.",
        "HttpRequest": "Eine Anfrage an einen Server: Methode (GET, POST …), Adresse und womöglich ein Inhalt.",
        "HttpResponse": "Die Antwort eines Servers: Statuscode und Inhalt.",
        "URI": "Eine Adresse im Netz, etwa http://localhost:8080/hallo.",
        "InetSocketAddress": "Eine Netzwerkadresse aus Rechner und Port.",
        "OutputStream": "Ein Strom, in den man Bytes schreibt – in eine Datei oder übers Netz.",
        "Connection": "Eine offene Verbindung zu einer Datenbank.",
        "DriverManager": "Öffnet Verbindungen zu Datenbanken – über eine JDBC-Adresse wie jdbc:sqlite:spiel.db.",
        "PreparedStatement": "Eine vorbereitete SQL-Abfrage: Werte kommen über Platzhalter (?) hinein – nie direkt in den SQL-Text.",
        "ResultSet": "Das Ergebnis einer Datenbank-Abfrage: eine Tabelle, die man mit next() Zeile für Zeile durchgeht.",
        "SQLException": "Der Alarm, wenn bei der Arbeit mit der Datenbank etwas schiefgeht.",
        "ObjectMapper": "Der JSON-Übersetzer der Bibliothek Jackson: Objekt → JSON-Text und zurück.",
    ...
    static let qualified: [String: String] = [
        ...
        "HttpServer.create": "Baut einen kleinen Webserver an einer Adresse mit Port (Port 0 = einen freien Port suchen lassen).",
        "HttpClient.newHttpClient": "Baut einen neuen HttpClient, der Anfragen verschickt.",
        "HttpRequest.newBuilder": "Beginnt den Bauplan (Builder) für eine Anfrage an eine Adresse; build() baut sie am Ende.",
        "HttpResponse.BodyHandlers": "Sagt send(), wie der Inhalt der Antwort gelesen wird – ofString(): als Text.",
        "HttpRequest.BodyPublishers": "Liefert den Inhalt, den eine POST-Anfrage mitschickt – ofString(): einen Text.",
        "URI.create": "Macht aus einem Text wie \"http://localhost:8080/hallo\" eine Adresse (URI).",
        "DriverManager.getConnection": "Öffnet eine Verbindung zu einer Datenbank; welche, sagt die JDBC-Adresse (etwa jdbc:sqlite:spiel.db).",
    ...
    static let methods: [String: (meaning: String, result: String?)] = [
        ...
        "GET": ("Im Bauplan einer Anfrage: Die Anfrage holt etwas ab (GET).", nil),
        "POST": ("Im Bauplan einer Anfrage: Die Anfrage schickt etwas mit (POST).", nil),
        "build": ("Baut aus einem Bauplan (Builder) das fertige Objekt.", "das fertige Objekt"),
        "ofString": ("Den Inhalt als Text lesen (BodyHandlers) oder als Text mitschicken (BodyPublishers).", nil),
        "send": ("Schickt eine Anfrage ab und wartet, bis die Antwort da ist.", "die Antwort des Servers"),
        "statusCode": ("Liefert den Statuscode einer HTTP-Antwort: 200 = alles gut, 404 = nicht gefunden.", "den Statuscode"),
        "body": ("Liefert den Inhalt einer HTTP-Antwort.", "den Inhalt der Antwort"),
        "method": ("Liefert die Methode einer Anfrage, etwa GET oder POST.", "die Methode der Anfrage"),
        "uri": ("Liefert die Adresse (URI) einer Anfrage.", "die Adresse der Anfrage"),
        "getPath": ("Liefert den Pfad einer Adresse – den Teil nach Server und Port, etwa /hallo.", "den Pfad"),
        "createContext": ("Legt fest, was ein Server unter einer Adresse (etwa /hallo) tut: Dort läuft dann der mitgegebene Handler.", nil),
        "stop": ("Hält einen Server an; die Zahl sagt, wie viele Sekunden er noch auf laufende Anfragen wartet.", nil),
        "getAddress": ("Liefert die Adresse, unter der ein Server lauscht.", "die Adresse des Servers"),
        "getPort": ("Liefert die Portnummer einer Adresse.", "die Portnummer"),
        "getRequestMethod": ("Liefert im Server die Methode der angekommenen Anfrage (GET, POST …).", "die Methode der Anfrage"),
        "getResponseBody": ("Liefert im Server den Strom, in den der Inhalt der Antwort geschrieben wird.", "den Strom für die Antwort"),
        "sendResponseHeaders": ("Schickt im Server den Anfang der Antwort: Statuscode und Länge des Inhalts (-1 = kein Inhalt).", nil),
        "getBytes": ("Macht aus einem Text Bytes – so gehen Texte übers Netz oder in Dateien.", "den Text als Bytes"),
        "write": ("Schreibt Bytes in einen Strom.", nil),
        "close": ("Schließt etwas, das Dinge belegt (Datei, Verbindung, Austausch) – danach ist es wieder frei.", nil),
        "lines": ("Zerlegt einen Text in seine Zeilen (als Stream).", "die Zeilen des Texts"),
        "prepareStatement": ("Bereitet eine SQL-Abfrage mit Platzhaltern (?) vor.", "die vorbereitete Abfrage"),
        "setString": ("Setzt einen Text für ein Fragezeichen der vorbereiteten Abfrage ein (gezählt ab 1).", nil),
        "setInt": ("Setzt eine ganze Zahl für ein Fragezeichen der vorbereiteten Abfrage ein (gezählt ab 1).", nil),
        "executeQuery": ("Führt eine SQL-Abfrage (SELECT) aus und liefert das Ergebnis als ResultSet.", "das Ergebnis der Abfrage"),
        "executeUpdate": ("Führt eine Änderung aus (INSERT, UPDATE, DELETE) und liefert, wie viele Zeilen betroffen waren.", "die Zahl der geänderten Zeilen"),
        "getString": ("Liest aus der aktuellen Zeile eines ResultSet einen Text – über den Spaltennamen oder die Spaltennummer (ab 1).", "den Text aus dieser Spalte"),
        "getInt": ("Liest aus der aktuellen Zeile eines ResultSet eine ganze Zahl – über den Spaltennamen oder die Spaltennummer (ab 1).", "die Zahl aus dieser Spalte"),
        "writeValueAsString": ("Jackson: macht aus einem Objekt einen JSON-Text.", "den JSON-Text"),
        "readValue": ("Jackson: baut aus einem JSON-Text wieder ein Objekt des angegebenen Typs.", "das zurückgebaute Objekt"),
        ...
        "next": ("Holt das Nächste: beim Scanner das nächste Wort, beim Iterator das nächste Element. Beim ResultSet rückt es auf die nächste Zeile vor und sagt, ob es noch eine gab (true/false).", "das nächste Wort"),
        ...
        "start": ("Startet: einen Thread (er läuft dann gleichzeitig) oder einen Server (er nimmt dann Anfragen an).", nil),
